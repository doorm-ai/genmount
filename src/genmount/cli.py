...
app.command(
    "init", help="Generate the device key + write config; optionally activate online."
)(init.run)
app.command("doctor", help="Check Python, Ollama, config, and device key health.")(doctor.run)
app.command(
    "chat", help="Send a prompt to the local model (default) or the cloud (--cloud)."
)(chat.run)
app.command(
    "sync", help="Download + install your account's models into Ollama (coming soon)."
)(sync.run)
